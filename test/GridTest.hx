...
/** Covers Grid's topology/query logic — no generated layout involved (see MazeGeneratorTest for that). **/
class GridTest extends Test {
	function testColumnsWrapAround():Void {
		var ringNeighbors = Grid.neighborsOf(RingNode(3, 0));
		var expected = RingNode(3, Grid.colsForRow(3) - 1);
	...
	function testNodeAtRingCellCenterReturnsThatCell():Void {
		...
		// Column phi is boundary-anchored (see Grid.centerOf's doc) — the
		// cell's actual center is at col+0.5, not col itself.
		var phi = 2 * Math.PI * (col + 0.5) / Grid.COLS;
	...
	function testNodeAtNearNorthPoleReturnsPoleNodeRegardlessOfPhi():Void {
		Assert.isTrue(Type.enumEq(Grid.nodeAt(0.0001, 0), PoleNode(North)));
		Assert.isTrue(Type.enumEq(Grid.nodeAt(0.0001, 5.5), PoleNode(North)));
	}

	function testNodeAtNearSouthPoleReturnsPoleNodeRegardlessOfPhi():Void {
		Assert.isTrue(Type.enumEq(Grid.nodeAt(Math.PI - 0.0001, 0), PoleNode(South)));
		Assert.isTrue(Type.enumEq(Grid.nodeAt(Math.PI - 0.0001, 5.5), PoleNode(South)));
	}

	function testNodeAtNearFullTurnReturnsLastColumn():Void {
		// Boundary-anchored columns (see Grid.centerOf's doc) floor cleanly:
		...
		Assert.isTrue(Type.enumEq(node, RingNode(row, Grid.COLS - 1)));
	}
	...
	function testColsForRowMatchesExpectedBandsForEveryRow():Void {
		...
		for (row in 1...(Grid.ROWS - 1)) {
			Assert.equals(expected[row - 1], Grid.colsForRow(row), 'row $row');
		}
	}
	...
	function testRowBoundaryNeighborsDoublingEntriesPartitionParentRangeExactly():Void {
		...
		for (boundary in boundaries) {
			var myCols = Grid.colsForRow(boundary.row);
			var otherCols = Grid.colsForRow(boundary.otherRow);
			...
			for (col in 0...myCols) {
				...
				var entries = Grid.rowBoundaryNeighbors(boundary.row, col, boundary.otherRow);
	...
	function testRowBoundaryNeighborsAtColumnExtremesStayInRange():Void {
		...
		for (boundary in boundaries) {
			...
			var first = Grid.rowBoundaryNeighbors(boundary.row, 0, boundary.otherRow);
			Assert.floatEquals(0, first[0].phiStart, 1e-9);

			var last = Grid.rowBoundaryNeighbors(boundary.row, myCols - 1, boundary.otherRow);
