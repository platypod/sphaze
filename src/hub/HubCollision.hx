...
/**
	...
	isn't built on that pipeline at all; a hand-placed convex hexagon needs
	nothing more than "did this step leave the hexagon", not wall-thickness
	zones, sliding, or per-edge open/closed state.
**/
class HubCollision {
	/**
		...
		`game.Collision`) if it would leave the hexagon — good enough for a
		small hand-placed room; revisit if a flat stop against a hub wall
		ever feels bad in practice.
	...
	**/
	public static function tryMove(player:Player, direction:h3d.Vector, distance:Float):Void {
		...
		player.moveAlong(direction, distance, maze.MazeGeometry.RADIUS);
